...
import { ShieldCheck, Menu, X, PlusCircle, ChevronRight, MessageSquare, LayoutDashboard, LogOut, Download } from "lucide-react";
...
export function Navbar() {
  ...
  const userRole = (user as { role?: string })?.role || currentUser?.role;
  const dashboardHref =
    ...
    <header className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 flex items-center justify-between h-16">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          {/* Quick Dashboard Link — Otomatis mendeteksi role login */}
          <Link href={dashboardHref}>
            <button className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 hover:text-blue-600 px-3 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 transition-colors cursor-pointer">
          ...
          <div className="pt-3 border-t border-slate-100 space-y-2">
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span>
                  Dashboard {userRole === "ADMIN" || userRole === "SUPER_ADMIN" ? "Admin" : "User"}
                </span>
              </div>
              <ChevronRight size={16} className="text-slate-400" />
            </Link>
            ...
            <button
              onClick={() => {
                setOpen(false);
                window.dispatchEvent(new CustomEvent("rekberin:open-pwa-install"));
              }}
              className="w-full flex items-center justify-between py-2.5 px-3 rounded-lg text-slate-700 hover:text-blue-600 hover:bg-blue-50 transition-colors text-sm font-medium cursor-pointer"
            >
              <div className="flex items-center gap-2.5">
                <Download size={16} className="text-blue-600" />
                <span>Install Aplikasi</span>
              </div>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">PWA</span>
            </button>
          </div>
